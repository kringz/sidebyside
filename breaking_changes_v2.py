...
def register_breaking_changes_routes(app):
    ...
    @app.route('/breaking_changes_v2', methods=['GET', 'POST'])
...
def create_breaking_changes_page(versions, app, compare_endpoint):
    ...
    def compare_versions_v2():
        ...
        try:
            ...
            logger.info(f"Compare request parameters: from={from_version}, to={to_version}")
            
            # Return a simple test response to confirm route works
            return jsonify({
                'success': True,
                'from_version': from_version,
                'to_version': to_version,
